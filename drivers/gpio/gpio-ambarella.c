...
/* Need to enable this function for Wi-Fi OOB. */
...
#if !defined(CONFIG_PLAT_AMBARELLA_AMBALINK)
/* Need to enable this part for Wi-Fi OOB. */
...
/* Need to enable this part for Wi-Fi OOB. */
		if (gpio_svc->gpio < 31) {
			boss_set_irq_owner(GPIO0_IRQ, BOSS_IRQ_OWNER_LINUX, 1);
		} else if (gpio_svc->gpio < 63) {
			boss_set_irq_owner(GPIO1_IRQ, BOSS_IRQ_OWNER_LINUX, 1);
		} else if (gpio_svc->gpio < 95) {
			boss_set_irq_owner(GPIO2_IRQ, BOSS_IRQ_OWNER_LINUX, 1);
		} else if (gpio_svc->gpio < 127) {
			boss_set_irq_owner(GPIO3_IRQ, BOSS_IRQ_OWNER_LINUX, 1);
		}
#endif
		break;
