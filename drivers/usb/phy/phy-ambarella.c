...
static inline void ambarella_switch_to_device(struct ambarella_phy *amb_phy)
{
	...
	amba_setbitsl(VIC0_REG(VIC_SENSE_OFFSET), 0x1);
	amba_setbitsl(VIC0_REG(VIC_EVENT_OFFSET), 0x1);
#endif
	if (amb_phy->owner_invert)
		amba_rct_setbitsl(amb_phy->own_reg, USB0_IS_HOST_MASK);
	else
...
#ifdef CONFIG_PLAT_AMBARELLA_AMBALINK
                /* Some boards do not have GPIO for port switching. */
...
#ifdef CONFIG_PLAT_AMBARELLA_AMBALINK
	/* Some boards do not has a GPIO for switcher. */
	/* Force USB0 to run device mode at first. */
	amb_phy->port_type = PORT_TYPE_DEVICE;
	amb_phy->phy_route = PHY_TO_DEVICE_PORT;
