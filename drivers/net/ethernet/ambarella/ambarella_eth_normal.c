...
static int ambeth_open(struct net_device *ndev)
{
	...
	if (ret_val)
        ...
        amba_writel(AHB_SCRATCHPAD_REG(AHBSP_CTL_OFFSET),
			amba_readl(AHB_SCRATCHPAD_REG(AHBSP_CTL_OFFSET))
					| AHBSP_ENET_GTX_CLK_POL_INVERT);
#endif
#endif
...
#if defined(CONFIG_PLAT_AMBARELLA_BOSS) && defined(CONFIG_PLAT_AMBARELLA_S3L)
	/* ref clk dir control */
	amba_writel(AHB_MISC_REG, amba_readl(AHB_MISC_REG) | 0x1 << 5);

	/* clk src select */
	amba_writel(ENET_CLK_SRC_SEL_REG, 0x01);

	/* clk invert */
	amba_setbitsl(AHB_SCRATCHPAD_REG(AHBSP_CTL_OFFSET), AHBSP_ENET_GTX_CLK_POL_INVERT);
#endif
