	...
	unsigned long val, void *data)
{
	...
	case AMBA_EVENT_POST_CPUFREQ:
		...
		aipc_mutex_lock(pinfo->ipc_mutex_id);
		...
		aipc_mutex_unlock(pinfo->ipc_mutex_id);
#endif
...
static int ambarella_i2c_probe(struct platform_device *pdev)
{
	...
	if (pinfo->irq < 0) {
        ...
        if ((u32) pinfo->regbase == IDC_BASE)
                pinfo->ipc_mutex_id = AMBA_IPC_MUTEX_I2C_CHANNEL0;
        else if ((u32) pinfo->regbase == IDC2_BASE)
                pinfo->ipc_mutex_id = AMBA_IPC_MUTEX_I2C_CHANNEL1;
        else if ((u32) pinfo->regbase == IDC3_BASE)
                pinfo->ipc_mutex_id = AMBA_IPC_MUTEX_I2C_CHANNEL2;
        else
                dev_err(&pdev->dev, "can't match correct amba ipc mutex ID at i2c%d!\n", pdev->id);
